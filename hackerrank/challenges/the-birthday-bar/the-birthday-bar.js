...
function solution(s, d, m) {
  ...
  for(let i=0; i < s.length; i++) {
    ...
    if( s.slice(i, m+i).reduce( (r,v) => r+=v, 0) === d) answer++;    
  }
  
  return answer; 
}

function solution1(s, d, m) {
  let answer = 0;
  ...
  for(let i=0; i < s.length; i++) {
    ...
    for(let j=i+1; j < m+i; j++) {
      sum += s[j];      
      console.log(`${i} / ${j} sum=${sum} ${d} ${answer} ${path}`)            
    }
...
console.log( solution([2,2,1,3,2], 4,2)); // 2
console.log ( solution([1,2,1,3,2], 3, 2)); // 2
console.log ( solution([1,1,1,1,1,1], 3, 2)); // 0
console.log ( solution([4], 4, 1)); // 1
console.log ( solution([2, 5, 1, 3, 4, 4, 3, 5, 1, 1, 2, 1, 4, 1, 3, 3, 4, 2, 1], 18, 7)); // 3
