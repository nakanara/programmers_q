...
function solution(arr) {
  ...
  let arrCount = new Array(6).fill(0);

  arr.forEach( v => arrCount[v]++);

  //let max = Math.max(...arrCount);
  
  arrCount.reduce((r,v, idx) => {
    if(r<v) {      
      answer = idx;
      return v;      
    } else {
...
function solution1(arr) {
  ...
  //let max = Math.max(...arrCount);
  const fnMax = (arr) => arr.reduce((r,v) => (r>v)? r:v, 0);  

  let max = fnMax(arrCount);
  console.log('max=' + max);
  answer = arrCount.indexOf(max); 
...
console.log( solution([1,4,4,4,5,2])); // 4
console.log( solution([1, 2, 3, 4, 5, 4, 3, 2, 1, 3, 4])); // 3
