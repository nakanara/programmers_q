...
function solution(record) {
  ...
  const DEF_ENTER = 'Enter', DEF_LEAVE = 'Leave', DEF_CHANE = 'Change';
  ...
  record.forEach( msg => {
    ...
    switch(token[0]) {
      case DEF_ENTER:
          history.push({'userId': token[1], 'action': DEF_ENTER});
          userMap[token[1]] = token[2];
      break;

      case DEF_LEAVE: 
        history.push({'userId' : token[1], 'action':DEF_LEAVE})
      break;
  ...
  history.forEach( his => {
    switch(his.action){
      case DEF_ENTER: answer.push(userMap[his.userId]+'님이 들어왔습니다.'); break;
      case DEF_LEAVE: answer.push(userMap[his.userId]+'님이 나갔습니다.'); break;
    }
  })
