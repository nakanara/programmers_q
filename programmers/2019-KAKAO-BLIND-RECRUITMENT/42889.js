...
function solution(N, stages) {
  let answer = [];
  let stage_users = new Array(N+2).fill(0);
  ...
  for(let i=1; i < N+1; i++) { // 3N
    let cnt = stage_users[i];
    answer.push({cnt: cnt, rate: ((cnt===0)? 0 : (cnt/totalCnt)), i:i });
    totalCnt -=cnt;
  }
  ...
  for(let i=0; i < N; i++) {
    for(let j=i+1; j < N; j++) {
      if( (answer[i].rate < answer[j].rate) || (answer[i].rate === answer[j].rate && answer[i].i > answer[j].i)) {
        tmp = answer[i];
        answer[i] = answer[j];
        answer[j] = tmp;
      }
