...
참고: https://it-and-life.tistory.com/15
...
function solution1(relation) {
  ...
  const generalKeys = (N) => {
    ...
    for(let i=1; i <= N; i++) { // keys size

      for(let j=0; j < N; j ++) {

        let arrCols = [];
        arrCols.push(j);
        for(let z=j+1; z < j+i && z < N; z++) {
          arrCols.push(z);
        }

        // 정상적 Key생성시
        if(arrCols.length === i)      result.push(arrCols);
      }
    }
  ...
  // 비교 대상 문자 생성
  const getPath = (keys, row) => keys.reduce( (r,v) => r += relation[row][v] +'/', '');
  
  // 키 여부 판정
  const checkIsKey = (keys) => {

    // 1. 전체 row 확인.
    let vector = new Map();

    for(let row=0; row < rowSize; row++) {
      let c = getPath(keys, row);
      vector.set(c, c); 
    }

    return (vector.size === rowSize)? true : false;
  }

  // 후보 키 존재 여부 판정
  const isCandidateKey = (keys) => {
    let duplicateCnt = 0;
    let ansSize = answer.length;

    for(let i=0; i < ansSize; i++) {
      duplicateCnt = 0;
      for(let j=0; j < answer[i].length; j++) {
        for(let z=0; z < keys.length; z++) {
          if(answer[i][j] === keys[z]) duplicateCnt++;
        }
      }

      if(duplicateCnt === answer[i].length) return false;
    }

    return true;
  }
  // 1. 비교 대상 그룹 생성
  candiCase = generalKeys(colSize);
  
  // 2. 비교
  for(let i=0; i < candiCase.length; i++) {
    let c = candiCase[i];
    
    // 2-1 이미 존재하고 있는 Key가 포함되어 있는지 판단
...
//console.log( solution([['a','b','c'], [1,'b','c'], ['a','b',4], ['a',5,'c']]));//1
