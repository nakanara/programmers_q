...
function solution(jobs) {
  ...
  // 시작 시간 map 을 담는다면 더 줄일 수 있음.
  let arrJobs = jobs.map((v) => {
    minStartTime = Math.min(minStartTime, v[0]);
    return {'start':v[0], 'time': v[1]};
  });
  ...
  while(isRun) {
    ...
    let work = getNextWork(timer);

    if(work) {
      answer += (timer-work.start) + work.time;  // 실제 작업 시간 
      timer += work.time;
    } else {
      timer++;
    }
